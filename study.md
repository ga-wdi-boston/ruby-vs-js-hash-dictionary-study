...
dale[:hobbies] = ['drinking bear', 'conspiracy theories']
```
...
```ruby
puts "My name is #{dale[:given_name]} #{dale[:family_name]} and I'm an #{dale[:occupation]} that enjoys
#{dale[:hobbies]}."
```
...
```javascript
let hankKeys = "hank's keys"
let hankValues = "hank's Values"
```
...
```javascript
console.log('My name is ' + hank.given_name + ' ' + hank.family_name +
' and Im a ' + hank.occupation + ' that enjoys ' + hank.hobbies[0] + '.')
```
