...
```ruby
# adds the key 'middle_name' with the value "Alvin" as a string to the dale hash
dale.store(:middle_name, "Alvin")
# adds the 'hobbies' key to the dale hash
dale.store(:hobbies, ['drinking beer', 'conspiracy theories'])
```
...
const hank = {
  familyName: 'hill',
  givenName: 'hank',
  occupation: 'propane and propane accerssories salesman'
  }
```
...
```javascript
"My name is ${hank.givenName} ${hank.familyName} and I\'m a ${hank.occupation} salesman that enjoys ${hank.hobbies[0]}"
```
