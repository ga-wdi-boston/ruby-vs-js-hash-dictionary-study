...
```ruby
my_new_string = "My name is #{dale[:given_name]} #{dale[:family_name]} and Im an #{dale[:occupation]} that enjoys #{dale[:hobbies][1]}."
```
...
```javascript
const myNewString = 'My name is ' + hank['giveName'] + ' ' + hank['familyName'] + 'and I''m a ' + hank['hobbies'][1] + ' that enjoys ' + hank['hobies'][0]
```
