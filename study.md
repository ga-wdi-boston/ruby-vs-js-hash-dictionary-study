...
```ruby
puts "My name is #{dale[:given_name]} #{dale[:family_name]} and I'm an #{dale[:occupation]} who enjoys #{dale[:hobbies][0]} and #{dale[:hobbies][1]}"
```
...
```javascript
let hank = {familyName: 'hill', givenName: 'hank', occupation: 'propane and propane accessories salesman'}
```
...
```javascript
let hank = "My name is " + hank.givenName + ' ' + hank.familyName + " and Im a " + hank.occupation + " salesman that enjoys " + hank.hobbies[0]
```
