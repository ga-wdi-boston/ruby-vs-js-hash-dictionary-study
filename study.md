...
```ruby
my-string = "MY names is #{dale[:"given-name"]} #{dale[:family_name]} and I'm
an #{dale[:occupation]} that enjoys conspiracy theories."
```
...
```javascript
const myString = 'My name is ${hank.family_name} and I\'m a ${hank.occupation}
that enjoys ${hank.hobbies[0]}'
```
