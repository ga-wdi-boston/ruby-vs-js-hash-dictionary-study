...
dale[:hobbiles] = ["drinking beer", "conspiracy theories"]
...
```ruby
"My name is #{dale[:game_name]} #{dale[:family_name]} and I'm an #{dale[:occupation]} that enjoys #{dale[:hobbiles[0]]} and #{dale[:hobbiles[1]]}"
```
...
```javascript
let response = "My name is " + hank.given_name + " and I'm a " + hank.occupation + " that enjoys " + hank.hobbies[0];
```
