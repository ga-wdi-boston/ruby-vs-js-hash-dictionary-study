...
```ruby
"My name is #{dale[:given_name]} #{dale[:family_name]} and I'm an #{dale[:occupation]} that enjoys conspiracy theories."
```
...
```javascript
let hanksKeys = Object.keys(hank);
let hanksValues = Object.values(hank);
```
...
```javascript
`My name is ${hank['given_name']} and I'm a ${hank['hobbies'][1]} that enjoys ${hank['hobbies'][0]}`;
```
