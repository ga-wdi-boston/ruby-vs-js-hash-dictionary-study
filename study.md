...
dale[:hobbies] = ["dingking beer", "conspiracy theories" ]
```
...
```ruby
dale_keys = dale.keys
dale_values = dale.values.flatten
```
...
```ruby
"My name is #{dale[:given_name]} #{dale[:famil_name]} and I'm an #{dale[:occupation]} that enjoys
#{dale[:hobbies[1]]}."
```
...
```javascript
`My name is ${hank.givenName} ${hank.familyName} Hill and I'm a ${hank.occupation} that enjoys ${hank.hobbies[0]}.`
```
