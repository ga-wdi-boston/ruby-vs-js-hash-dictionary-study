...
```ruby
dale.merge!(:middle_name => 'Alvin', :hobbies => ['drinking beer', 'conspiracy theories'])
```
...
```ruby
dale.merge!(:family_name => "Gribble", :given_name => 'Dale')
```
...
```ruby
"my name is #{dale.values[1]} #{dale.values[0]} and I'm and #{dale.values[2]} that enjoys #{dale.values[4][1]}"
```
...
```javascript
hank = {familyName : "Hill", givenName : "Hank"}
```
...
```javascript
console.log("My name is " + hank.givenName+" " +hank.familyName + " and I'm a " + hank.occupation + " " + "that enjoys " + hank.hobbies[0])
```
