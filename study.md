...
```ruby
dale[:family_name].capitalize!
dale[:'given-name']. capitalize!
```
...
```ruby
"My name is #{dale[:'given-name'].capitalize} #{dale[:family_name].capitalize} and I'm an #{dale[:occupation]} that enjoys #{dale[:hobbies][1]}."
```
...
let hank = {
  family_name: 'hill',
  'given-name': 'hank',
  occupation: 'propane and propane acessories salesman'
}
```
...
```javascript
hank.family_name = hank.family_name[0].toUpperCase()+hank.family_name.slice(1)
hank['given-name'] = hank['given-name'][0].toUpperCase()+hank['given-name'].slice(1)
```
...
let hankValues = Object.values(hank) // experimental command, apparently
let stripValues = function (obj) {
  let values = []
  for (let key in obj) {
    values.push(obj[key])
  }
...
```javascript
`My name is ${hank['given-name'][0].toUpperCase()+hank['given-name'].slice(1)} ${hank.family_name = hank.family_name[0].toUpperCase()+hank.family_name.slice(1)} and I'm a ${hank.occupation} that enjoys ${hank.hobbies[0]}.`
```
