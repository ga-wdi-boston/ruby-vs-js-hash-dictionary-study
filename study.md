...
```ruby
dale_says = "My name is #{dale[:given_name]} and I'm an #{dale[:occupation]} that enjoys #{dale[:hobbies][1]}."
```
...
```javascript
let bld = `My name is ${hank.given_name} ${hank.family_name} and I\'m a ${hank.occupation} that enjoys ${hank.hobbies[0]}.`
```
