...
# Note that I had to change the hyphen to an underscore in given_name.  The hyphen was causing errors. Not sure if that was left in there intentionally.
```
...
```ruby
story = "My name is #{dale[:given_name]} #{dale[:family_name]} and I'm an #{dale[:occupation]} that enjoys #{dale[:hobbies][1]}."
```
...
```javascript
hankKeys = Object.keys(hank)
hankValues = Object.values(hank)
```
...
```javascript
let story = `My name is ${hank.given_name} ${hank.family_name} and I'm a ${hank.occupation} that enjoys ${hank.hobbies[0]}.`
```
