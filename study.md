...
```ruby
dale[:family_name] = dale[:family_name].capitalize!
dale[:given_name] = dale[:given_name].capitalize!
```
...
```javascript
hank.family_name = hank.family_name.charAt(0).toUpperCase() + hank.family_name.slice(1);
hank.family_name = hank.given_name.charAt(0).toUpperCase() + hank.given_name.slice(1);
```
