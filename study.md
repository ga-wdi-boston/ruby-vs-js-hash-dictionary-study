...
```javascript
hank.hankKeys = Object.keys(hank)
hank.hankValues = Object.keys(hank).map((key) => {
  return hank[key]
})
```
...
```javascript
const string = `My name is ${hank['given-name']} ${hank.family_name} and I'm a ${hank.hobbies[1]} salesman that enjoys ${hank.hobbies[0]}.`
```
