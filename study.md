...
```ruby
dale = {"Key" => "Value", "---" => "---", "family_name" => "gribble", "given-name" => "dale", "occupation" => "exterminator"}
```
...
dale.update(dale) {|family_name,Gribble| f(gribble)}
dale.update(family_name:"Gribble")
dale.update(given-name:"Dale")
```
...
```ruby
dale = "My name is Dale Gribble and I'm an exterminator that enjoys conspiracy theories."
```
...
```javascript
hank = "My name is " + hank.given-name + hank.family_name + " and I'm a propane and propane accesories salesman that enjoys drinking beer."
```
