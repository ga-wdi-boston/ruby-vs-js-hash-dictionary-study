...
```ruby
p `my name is #{dale[:given_name]} #{dale[:family_name]} and i'm an #{dale[:occupation]} that enjoys #{dale[:hobbies][1]}.`
```
...
cant figure out the values one... js is not as practical as ruby.
the mozilla deloper says Object.values(obj) but I have not made it work yet.
```
...
```javascript
str = "My name is " + hank['given-name'] + " " + hank.family_name + " and I'm a " + hank.occupation + " that enjoys " + hank.hobbies[0] + "."
```
