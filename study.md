...
```ruby
//Not really sure how to distinguish betweek keys and values
dale.dale_keys
dale.dale_values
```
...
```ruby
prints "My name is #{dale[:given_name]} #{dale[:family_name]} and I'm an #{dale[:occupation]}
that enjoys #{dale[:hobbies][1]}."
```
...
hank.hobbies = ["drinking beer", "propane and prpoane accessories"]
```
...
and I'm a " + hank.occupation + " that enjoys " + hank.hobbies[0] + ".")
```
